...
public class DataSender : MonoBehaviour
{
    ...
    private static string url = "https://citmalumnes.upc.es/~hangx/Importers.php";
    ...
    IEnumerator SendNewPlayer(DateTime date)
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.Post("https://citmalumnes.upc.es/~hangx/Importers.php", form))
        {
            ...
            {
                string response = www.downloadHandler.text.Trim(new char[] { '\uFEFF', '\u200B', ' ', '\t', '\r', '\n' });
                if (uint.TryParse(response, out uint parsedId) && parsedId > 0)
                {
                    playerID = (int)parsedId;
                    Debug.Log("New player ID received: " + playerID);
                }
                else
                {
                    Debug.LogError("Invalid player ID received: " + response);
                }
            }
        }
    }
